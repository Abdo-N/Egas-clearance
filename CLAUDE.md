...
egas-clearance/
  ...
  frontend/
    src/
      ...
      components/            ChecklistPanel, LanguageToggle
...
Two rules are enforced in `backend/src/routes/request.routes.js`
(`PATCH /:id/departments/:deptKey/items/:itemKey`), and ONLY there:

1. Items within a department must be checked in ascending `order`.
2. A department with `isFinal: true` (only IT, right now) cannot have its LAST
   item checked until every other department on the request has `status: "completed"`.
   This is what makes "IT signs last" work without ever hardcoding the string "it"
   in the gating logic itself.

If you need to change this logic, it lives in exactly one place. Don't duplicate
it in the frontend beyond the UX hint in `ChecklistPanel.jsx` — the frontend check
there is cosmetic; the backend is the source of truth.
